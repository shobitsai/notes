    ...
    public static void main(String[] args) {
        ...
        for (int i = k - 1; i < n; i++) {
            ...
            for (Map.Entry<Integer, Integer> e : map.entrySet()) {
                int[] x = {e.getKey(), e.getValue()};
                lx.add(x);
            }

            Collections.sort(lx, (a, b) -> {
                int x = a[1] * (int) Math.pow(a[0], F);
                int y = b[1] * (int) Math.pow(b[0], F);
            ...
            int temp = 0;
            for (int a = 0; a < X; a++)
                temp += lx.get(a)[0] * lx.get(a)[1];

            ans.add(temp);

            int preFreq = map.get(arr[i - k + 1]);
            if (preFreq > 0)
                map.put(arr[i - k + 1], preFreq - 1);
            else
