...
class D02P1 {
    ...
    public static int solve(int[] task, int[] time, int k, int low, int upper, int wg, int n) {
        int taskWindow = 0, timeWindow = 0, ans = 0;
        ...
        for (int i = k - 1; i < n; i++) {
            ...
            if (taskWindow < low && timeWindow >= wg) {
                ans -= 2;
            } else if (taskWindow < low && timeWindow < wg) {
                ans--;
            } else if (taskWindow >= upper && timeWindow >= wg) {
                ans++;
            } else if (taskWindow >= upper && timeWindow < wg) {
                ans += 2;
            }

            taskWindow -= task[i - k + 1];
            timeWindow -= time[i - k + 1];
        }
    ...
    public static void main(String[] args) {
        ...
        int[] task = takeInput(n, sc);
        int[] time = takeInput(n, sc);
        ...
        int ans = solve(task, time, k, low, upper, workGoal, n);
